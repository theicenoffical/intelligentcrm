...
export default function Developers() {
  return (
    <>
      ...
      />
      <section className="border-b border-white/8" data-testid="developers-capabilities">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                {CAPABILITIES.map((c, i) => (
                  <Reveal key={c.title} delay={(i % 2) * 0.06}>
                    <div className="border border-white/8 rounded-md p-6 hover:border-white/20 transition-colors duration-300" data-testid={`dev-cap-${i}`}>
                      <h3 className="font-display text-base font-bold text-[#F2F2F2]">{c.title}</h3>
                      <p className="text-[#8F8F9D] text-xs mt-2 leading-relaxed">{c.desc}</p>
                    </div>
                  </Reveal>
                ))}
              </div>
            ...
            <Reveal delay={0.15}>
              <div className="lg:sticky lg:top-28">
                <div className="rounded-md border border-white/10 bg-[#0C0C0F] overflow-hidden" data-testid="dev-code-sample">
                  <div className="flex items-center gap-2 px-5 py-3.5 border-b border-white/8">
                    ...
                    <span className="font-mono2 text-[10px] tracking-[0.2em] text-[#8F8F9D] ml-2">CREATE OPPORTUNITY</span>
                  </div>
                  <pre className="p-6 overflow-x-auto">
                    <code className="font-mono2 text-xs leading-relaxed text-[#c9c9d4]">{CODE}</code>
                  </pre>
                </div>
                <p className="font-mono2 text-[10px] tracking-[0.2em] text-[#8F8F9D]/60 mt-4 uppercase">
