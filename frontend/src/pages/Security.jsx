...
export default function Security() {
  return (
    <>
      ...
      />
      <section className="border-b border-white/8" data-testid="security-capabilities">
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
            {SECURITY_CAPABILITIES.map((c, i) => (
              <Reveal key={c.title} delay={(i % 4) * 0.06}>
                <div className="surface-card rounded-md p-7 h-full hover:border-white/20 transition-colors duration-300" data-testid={`security-cap-${c.title.toLowerCase().replace(/[^a-z]+/g, "-")}`}>
                  <ShieldCheck className="w-4 h-4 text-[#FF3333] mb-5" />
                  <h3 className="font-display text-base font-bold text-[#F2F2F2]">{c.title}</h3>
                  <p className="text-[#8F8F9D] text-xs mt-3 leading-relaxed">{c.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="border-b border-white/8 bg-[#0C0C0F]" data-testid="security-deployment">
        <div className="container-x py-20 md:py-28">
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-px bg-white/8 border border-white/8">
            {DEPLOYMENT_MODELS.map((m, i) => (
              <Reveal key={m.name} delay={(i % 4) * 0.06} className="bg-[#0C0C0F]">
                <div className="p-8 h-full" data-testid={`security-deploy-${m.name.toLowerCase().replace(/\s+/g, "-")}`}>
                  <span className="font-mono2 text-[11px] text-[#FF3333] tracking-widest">MODEL {m.num}</span>
                  <h3 className="font-display text-lg font-bold text-[#F2F2F2] mt-3">{m.name}</h3>
