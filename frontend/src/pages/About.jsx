...
export default function About() {
  return (
    <>
      ...
      <section className="border-b border-white/8" data-testid="about-story">
        <div className="container-x py-20 md:py-28 grid lg:grid-cols-2 gap-16">
          ...
          <Reveal delay={0.1}>
            <div className="space-y-6 text-[#8F8F9D] text-sm md:text-base leading-[1.85]">
              <p>
      ...
      <section className="border-b border-white/8 bg-[#0C0C0F]" data-testid="about-values">
            ...
            {VALUES.map((v, i) => (
              <Reveal key={v.num} delay={(i % 2) * 0.08}>
                <div className="surface-card rounded-md p-10 h-full" data-testid={`value-${v.num}`}>
                  <span className="font-mono2 text-sm text-[#FF3333] tracking-widest">{v.num}</span>
                  <h3 className="font-display text-2xl font-bold text-[#F2F2F2] mt-4 tracking-[-0.02em]">{v.title}</h3>
                  <p className="text-[#8F8F9D] text-sm mt-4 leading-relaxed">{v.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="border-b border-white/8" data-testid="about-office">
        <div className="container-x py-20 md:py-28 grid lg:grid-cols-2 gap-16 items-start">
          ...
          <Reveal delay={0.1}>
            <div className="surface-card rounded-md p-10">
              <p className="font-mono2 text-[11px] tracking-[0.25em] uppercase text-[#8F8F9D] mb-6">Office</p>
              <p className="font-display text-xl font-bold text-[#F2F2F2] leading-snug" data-testid="about-address">{SITE.address}</p>
              <div className="mt-8 pt-8 border-t border-white/8 space-y-3">
                <a href={SITE.phoneHref} data-testid="about-phone" className="block text-[#8F8F9D] hover:text-white text-sm transition-colors duration-150">{SITE.phone}</a>
                <a href={`mailto:${SITE.email}`} data-testid="about-email" className="block text-[#8F8F9D] hover:text-white text-sm transition-colors duration-150">{SITE.email}</a>
              </div>
            </div>
          </Reveal>
        </div>
