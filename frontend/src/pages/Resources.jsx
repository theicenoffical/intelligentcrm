        ...
        <div className="grid md:grid-cols-2 gap-6">
          {cards.map((c, i) => (
            <Reveal key={c.title} delay={i * 0.06}>
              <Link to={c.to} data-testid={c.testid} className="group surface-card rounded-md p-10 block h-full hover:border-white/20 transition-colors duration-300">
                <c.icon className="w-5 h-5 text-[#FF3333] mb-6" />
                <h2 className="font-display text-2xl font-bold text-[#F2F2F2] tracking-[-0.02em]">{c.title}</h2>
                <p className="text-[#8F8F9D] text-sm mt-3 leading-relaxed">{c.desc}</p>
                <span className="inline-flex items-center gap-2 text-sm text-white mt-6 font-medium">
        ...
        <div className="container-x py-20 md:py-28">
          ...
          <div className="space-y-px bg-white/8 border border-white/8">
            {BLOG_POSTS.slice(0, 3).map((p) => (
              <Link key={p.slug} to={`/resources/blog/${p.slug}`} data-testid={`res-post-${p.slug}`} className="group bg-[#0C0C0F] flex flex-col md:flex-row md:items-center justify-between gap-4 p-8 hover:bg-[#141418] transition-colors duration-200">
                <div>
                  <p className="font-mono2 text-[10px] tracking-[0.25em] uppercase text-[#FF3333] mb-2">{p.category} · {p.readTime}</p>
                  <h3 className="font-display text-xl font-bold text-[#F2F2F2] tracking-[-0.01em]">{p.title}</h3>
                </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {filtered.map((p, i) => (
            <Reveal key={p.slug} delay={(i % 2) * 0.06}>
              <Link to={`/resources/blog/${p.slug}`} data-testid={`blog-card-${p.slug}`} className="group surface-card rounded-md p-10 block h-full hover:border-white/20 transition-colors duration-300">
                <div className="flex items-center gap-4 mb-6">
                  <span className="font-mono2 text-[10px] tracking-[0.25em] uppercase text-[#FF3333]">{p.category}</span>
                  <span className="font-mono2 text-[10px] text-[#8F8F9D]/60">{p.date} · {p.readTime}</span>
                </div>
                <h2 className="font-display text-2xl font-bold text-[#F2F2F2] tracking-[-0.02em] leading-snug">{p.title}</h2>
                <p className="text-[#8F8F9D] text-sm mt-4 leading-relaxed">{p.excerpt}</p>
                <span className="inline-flex items-center gap-2 text-sm text-white mt-8 font-medium">
    ...
    <>
      ...
      <article className="container-x pt-36 pb-20 md:pt-44" data-testid="blog-post">
        <div className="max-w-3xl mx-auto">
          <Link to="/resources/blog" data-testid="back-to-blog" className="inline-flex items-center gap-2 font-mono2 text-[11px] tracking-[0.25em] uppercase text-[#8F8F9D] hover:text-white transition-colors duration-150">
            <ArrowLeft className="w-3.5 h-3.5" /> Journal
          </Link>
          <Reveal>
            <div className="flex items-center gap-4 mt-10 mb-8">
              <span className="font-mono2 text-[10px] tracking-[0.25em] uppercase text-[#FF3333]">{post.category}</span>
              <span className="font-mono2 text-[10px] text-[#8F8F9D]/60">{post.date} · {post.readTime} read</span>
            </div>
            <h1 className="font-display font-extrabold text-3xl md:text-5xl tracking-[-0.03em] leading-[1.05] text-[#F2F2F2]">{post.title}</h1>
            <p className="text-[#8F8F9D] text-base md:text-lg mt-6 leading-relaxed">{post.excerpt}</p>
          </Reveal>
          <div className="mt-12 pt-12 border-t border-white/8 space-y-8">
            {post.body.map((para, i) => (
              <Reveal key={i} delay={Math.min(i * 0.05, 0.2)}>
                <p className="text-[#c9c9d4] text-base md:text-lg leading-[1.85]">{para}</p>
              </Reveal>
            ))}
          </div>
          <div className="mt-16 pt-10 border-t border-white/8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <p className="text-[#8F8F9D] text-sm max-w-sm">Evaluating CRM platforms? We'll walk through your requirements directly.</p>
  ...
  return (
    <>
      ...
      <section className="container-x py-20 md:py-28" data-testid="case-studies-list">
        <p className="font-mono2 text-[11px] tracking-[0.25em] uppercase text-[#FF3333] mb-10" data-testid="illustrative-label">
          ...
          {scenarios.map((s, i) => (
            <Reveal key={s.title} delay={i * 0.08}>
              <div className="surface-card rounded-md p-8 h-full" data-testid={`scenario-${i}`}>
                <p className="font-mono2 text-[10px] tracking-[0.25em] uppercase text-[#8F8F9D] mb-4">{s.industry}</p>
                <h3 className="font-display text-xl font-bold text-[#F2F2F2] tracking-[-0.01em] leading-snug">{s.title}</h3>
                <p className="text-[#8F8F9D] text-sm mt-4 leading-relaxed">{s.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
        <Reveal>
          <div className="mt-16 border border-white/8 rounded-md p-10 text-center" data-testid="case-study-cta">
            <h3 className="font-display text-2xl font-bold text-[#F2F2F2]">Running an evaluation?</h3>
            <p className="text-[#8F8F9D] text-sm mt-3 max-w-md mx-auto">We'll share reference architectures and walk through deployment scenarios relevant to your industry.</p>
            <div className="mt-8">
